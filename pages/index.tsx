...
export default function UnloktLanding() {
  ...
  const NavLink = ({ label }: { label: string }) => (
    <a href={`#${label.toLowerCase().replaceAll(" ", "-")}`} className="text-sm font-medium text-zinc-700 hover:text-zinc-900">{label}</a>
  );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              <NavLink label="FAQ" />
              <a href="/locksmiths" className="text-sm font-medium text-zinc-700 hover:text-zinc-900">
  For Locksmiths
</a>
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <a href="#" className={`inline-flex items-center justify-center px-3 py-2 text-sm font-semibold rounded-xl ${theme.accent} hover:brightness-95 shadow-sm`}>
                Get the app
              </a>
            </div>
            <button onClick={() => setMobileOpen(v=>!v)} className="md:hidden inline-flex items-center justify-center h-10 w-10 rounded-xl border border-zinc-200">
          ...
          <div className="md:hidden border-t border-zinc-200">
            <div className="px-4 py-3 grid gap-3">
              {['How it works','Services','Pricing','Coverage','FAQ'].map(l=> (
                <a key={l} href={`#${l.toLowerCase().replaceAll(' ','-')}`} className="text-sm font-medium py-2">{l}</a>
              ))}
  ...
  For Locksmiths
      ...
      <section id="hero" className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className={`mt-4 ${theme.subtext} max-w-xl`}>Tap once to request a vetted pro. Upfront pricing, live ETAs, and cashless checkout. No more mystery phone trees or bait-and-switch quotes.</p>
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <a href="#" className={`inline-flex items-center justify-center px-4 py-3 text-sm font-semibold rounded-xl ${theme.accent} hover:brightness-95 shadow`}>Get the app</a>
              <a href="#how-it-works" className="inline-flex items-center justify-center px-4 py-3 text-sm font-semibold rounded-xl border border-zinc-300 bg-white hover:bg-zinc-50">See how it works</a>
            </div>
                ...
                <div className="mt-4 grid gap-3">
                  {[
                    ...
                    <div key={i} className="p-4 rounded-2xl bg-zinc-800 border border-zinc-700">
                      ...
                      <button className="mt-3 w-full rounded-xl bg-amber-400 text-amber-900 text-sm font-semibold py-2">Select</button>
                    </div>
                  ))}
                </div>
      ...
      <section id="services" className="py-20 bg-white border-y border-zinc-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between">
            ...
            <a href="#" className="text-sm font-medium text-zinc-700 hover:text-zinc-900">See all</a>
          </div>
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={idx} className={`p-6 rounded-2xl ${theme.card} border ${theme.border} shadow-sm hover:shadow-md transition-shadow`}>
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className={`p-6 rounded-2xl ${theme.card} border ${theme.border} shadow-sm flex flex-col`}>
                ...
                <a href="#" className={`mt-6 inline-flex items-center justify-center px-3 py-2 text-sm font-semibold rounded-xl ${theme.accent} hover:brightness-95`}>Get started</a>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <p className={`mt-3 ${theme.subtext}`}>Operating in 120+ metros with a growing network of licensed locksmiths. Average acceptance time: under 60 seconds.</p>
              <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
                {['24/7 dispatch','Background-checked','Insurance verified','Parts warranty'].map((b, i)=> (
                  <div key={i} className={`p-3 rounded-xl ${theme.card} border ${theme.border}`}>✅ {b}</div>
                ))}
              </div>
            </div>
            <div className={`p-6 rounded-2xl ${theme.card} border ${theme.border} shadow-sm`}>
              <div className="h-56 rounded-xl bg-gradient-to-br from-zinc-100 to-zinc-200 flex items-center justify-center text-zinc-500">Map placeholder</div>
      ...
      <section className="py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`p-6 md:p-8 rounded-3xl ${theme.card} border ${theme.border} shadow-sm flex flex-col md:flex-row items-center justify-between gap-6`}>
            <div>
              <h3 className="text-xl font-semibold">Ready when you are</h3>
              <p className={`mt-1 ${theme.subtext}`}>Download Unlokt to request help instantly or schedule a time that works for you.</p>
            </div>
            <div className="flex items-center gap-3">
              <a href="#" className={`inline-flex items-center justify-center px-4 py-3 text-sm font-semibold rounded-xl ${theme.accent} hover:brightness-95`}>App Store</a>
              <a href="#" className="inline-flex items-center justify-center px-4 py-3 text-sm font-semibold rounded-xl border border-zinc-300 bg-white hover:bg-zinc-50">Google Play</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="py-12 border-t border-zinc-200 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-sm text-zinc-600 grid md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="mt-3 max-w-sm">The fastest, fairest way to get back in. 24/7 nationwide locksmiths with transparent pricing.</p>
          </div>
            ...
            {[
              ...
              <div key={i}>
                ...
                <ul className="mt-3 space-y-2">
                  {c.l.map((x)=> (<li key={x}><a href="#" className="hover:text-zinc-900">{x}</a></li>))}
                </ul>
              </div>
            ))}
          </div>
        </div>
        <div className="mt-8 text-center text-xs text-zinc-500">© {new Date().getFullYear()} Unlokt, Inc. All rights reserved.</div>
      </footer>
    </div>
  );
}
